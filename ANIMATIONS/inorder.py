...
def build_tree(nodes, index=0):
    if index < len(nodes):
        ...
        node.left = (
            build_tree(nodes, 2 * index + 1) if 2 * index + 1 < len(nodes) else None
        )
        node.right = (
            build_tree(nodes, 2 * index + 2) if 2 * index + 2 < len(nodes) else None
        )
        return node
    return None
...
def inorder_traversal(root, graph, pos, speed):
    ...
    traverse(root)
    print(f"Inorder Traversal: {result}")  # Print the output to the command line
    return result
...
inorder_traversal(root, graph, pos, 1.5)
plt.ioff()
plt.show()
